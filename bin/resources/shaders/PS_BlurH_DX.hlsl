...
// float4 main(PS_INPUT Input) : SV_Target {
	...
	// for(int i = 1; i < 3; ++i) {
		// fragmentColor += pow(InputTexture.Sample(lSampler, Input.TexCoord + (float2(offset[i], 0.0f) / ViewportDim.xy)), gamma) * weight[i];
		// fragmentColor += pow(InputTexture.Sample(lSampler, Input.TexCoord - (float2(offset[i], 0.0f) / ViewportDim.xy)), gamma) * weight[i];
	// }
...
float4 main(PS_INPUT input) : SV_TARGET0
{
  ...
  for(int i = 1; i < 3; i++)
  {
    FragmentColor += pow(InputImage.Sample(samLinear, input.TexCoord + (float2(offset[i], 0.0f) / float2((fViewportDimensions).x,(fViewportDimensions).y))), 1.0f) * weight[i];
	FragmentColor += pow(InputImage.Sample(samLinear, input.TexCoord - (float2(offset[i], 0.0f) / float2((fViewportDimensions).x,(fViewportDimensions).y))), 1.0f) * weight[i];
  }
